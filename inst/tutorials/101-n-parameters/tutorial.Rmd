...
### Exercise 17
<!--AR: How does the scenario, that we are a gubenatorial candidate in Texas, fit in? Should the Preceptor Table not refer to a question that occurs now and relates to that scenario? -->
...
<!--AR: Shouldn't the Preceptor Table relate to the original scenario, not the data? Units would be registered Texas voters in our campaigns database, outcomes would be whether an individual votes in the upcoming election -->

<!--AR: how do we choose key covariates at this stage? I would think that they would have to relate to the question being asked? Why is age included now and here when we're not yet sure, empirically, its valuable and the question of postcard messaging with respect to voting turnout doesn't explicitly include age?  -->
...
<!--AR: Here, the answer message refers to a Preceptor Table that refers to a future campaign. This is inconsistent with the Preceptor Table described above.-->
...
The exact time period used --- whether hour, day, month, year, or whatever --- is relatively arbitrary. The important thing to note is that the Population Table, unlike the Preceptor Table, covers a period of time over which things m

<!-- AR: Where are the rows from the Preceptor Table? -->
...
<!--AR: Should the answer relate to the Precptor Table units from Texas instead of data units from Michigan? -->
```{r justice-10}
question_text(NULL,
