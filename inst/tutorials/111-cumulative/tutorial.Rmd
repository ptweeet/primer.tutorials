...
# MASS and easystats must go first so that tidyverse is loaded after them and its version of select() stays active.
...
<!-- AR: This code chunk produces 2 errors -->
```{r}
...
Continue the pipe to `dplyr::select()` and have `approval`, `education`, and `ideology` inside.
...
ces |> filter(year == 2020) |> dplyr::select(approval, education, ideology) |>
  drop_na() |>
  slice_sample(n = 2000)
```
...
We can see some information about our data. Most people are moderate in terms of political ideology. Most people strongly disapprove the president, the second biggest group for that variable is strongly approve. Most people only have a high school as their highest education.
...
```{r justice-6}
question_text(NULL,
	message = "We are concerned about the year 2024, and the data we have has information for 2020. The dawn of the COVID-19 pandemic was in 2020 and it completely upended American life in such a way that political ideology may have been less important to presidential approval than in 2024 when the Pandemic had subsided and many societal restrictions had been lifted.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- AR: I'm skeptical of the previous answer message. Instructions ask to propose a reason might not hold, seems a little like a trick question to then say there are no representativeness concerns. -->
```{r justice-9}
question_text(NULL,
	message = "In the Preceptor Table, we. are concerned with approval ratings of prospective voters, while our population is the general US population. These two groups are likely differ in composition.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
where each $\theta_k$ defines a threshold between approval levels, and the model estimates the probability of being at or below each category using a series of cumulative log-odds.
...
Because our outcome variable is **cumulative**, start to create the model by using `polr()`. Arguments should be `formula = approval ~ ideology + education` and `data = x`.
...
Replace the `x` code with the following in your QMD. Run the code chunk. `Cmd/Ctrl + Shift + K`.
...
<!-- AR: Use of "console" is unclear here. Does it mean to run just in the tutorial in analysis.qmd? If in analysis.qmd, see comment below. -->
<!-- AR: output of this call in analysis.qmd results in a graph, not a table as shown in the rendered tutorial. Have to pipe into as.data.frame() the str(max.level = 1)to see table output there.-->
...
<!--AR: What are we caching in the code chunk? Currently, we don't add anything to it.-->
...
In your last code chunk, add `tidy()` on `fit_approval` with the argument `conf.int` set equal to `TRUE`. Save the result as `tidy_approval` Run the code chunk. The returns 95% intervals for all the parameters in our model. This takes a minute or two to run. This is why we cache the result.
...
Create a new code chunk in your QMD. Ask AI to help you make a nice looking table from the `tidy_approval`. You don't have to include all the variables which `tidy()` produces. We often just show the estimate and the confidence intervals.
